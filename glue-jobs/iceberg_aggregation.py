...
from pyspark.sql.functions import avg, col, count, countDistinct, date_trunc, sum
...
daily_stats = orders_df.groupBy(
    date_trunc("day", col("order_date")).alias("date"), col("status")
).agg(
...
daily_stats.writeTo(agg_table).option("mergeSchema", "true").overwritePartitions()
